...
// Name, Contact number, Problem, optional image upload) to Cloudinary + DB.
router.post("/", (req, res) => {
  upload.single("attachment")(req, res, async (uploadErr) => {
    ...
    try {
      const { sessionId, language, name, contactNumber, description } =
        req.body;

      if (!sessionId || !name || !contactNumber) {
        return res.status(400).json({
          success: false,
          message: "sessionId, name and contactNumber are required",
        });
      }
